...
do
    ...
    local function HandleEvent(self, e, ...)
        if e == "ADDON_LOADED" then
            LoadOptions()
            addon.Reserves:Load() -- <--- ADD THIS LINE
            addon.Reserves:RefreshWindow() -- Refresh the UI after loading
        end
...
do
    ...
    function Raid:AddLoot(msg, rollType, rollValue)
        ...
        if addon.ignoredItems[itemId] then return end
...
do
    ...
    local function AnnounceRoll(rollType, chatKey)
        ...
        local _, _, _, _, itemID = string.find(itemLink or "", "|?c?(%x*)|?H?([^:]*):?(%d+):?")
        itemID = tonumber(itemID)
...
do
    ...
    function Reserves:Save()
        print("KRT Reserves: Saving data. Content size: ", Utils.tableLen(reservesData))
        -- Use deepCopy here to ensure KRT_SavedReserves is a separate copy
        KRT_SavedReserves = table.deepCopy(reservesData)
    end

    function Reserves:Load()
        print("KRT Reserves: Loading data. KRT_SavedReserves is nil?", KRT_SavedReserves == nil)
        if KRT_SavedReserves then
            -- Use deepCopy here to ensure reservesData is a separate copy
            -- from the saved data, allowing modifications without affecting KRT_SavedReserves
            reservesData = table.deepCopy(KRT_SavedReserves)
            print("KRT Reserves: Data loaded. Content size: ", Utils.tableLen(reservesData))
        else
            -- Initialize if no saved data exists
            reservesData = {}
        end
    end

    function Reserves:ResetSaved()
        KRT_SavedReserves = nil -- Set the SavedVariables to nil to completely clear it from storage
        wipe(reservesData) -- This now only wipes the *local* copy
        self:RefreshWindow()
        self:CloseWindow()
    ...
    function Reserves:ParseCSV(csv)
        ...
        for line in csv:gmatch("[^\r\n]+") do
            ...
            if firstLine then
                ...
                local itemIdStr, playerName = line:match("^([^,]*),([^,]*)")
    ...
    function Reserves:UpdateReserveItemData(itemId, itemName, itemLink, itemIcon)
        ...
        for _, row in ipairs(rows) do
            ...
            if cached then
                local icon = cached.itemIcon or "Interface\\Icons\\INV_Misc_QuestionMark"
                local text = cached.itemLink or cached.itemName or ("Item ID: " .. itemId)
    ...
    function Reserves:RefreshWindow()
        ...
        for i, entry in ipairs(entries) do
            local row = Reserves:CreateReserveRow(scrollChild, entry, yOffset, i)
    ...
    function Reserves:CreateReserveRow(parent, info, yOffset, index)
        ...
        bg:SetAllPoints(row)
        bg:SetTexture(index % 2 == 0 and 0.1 or 0, 0.1, 0.1, 0.3)
        ...
        playerText:SetText(info.player or "")

        local quantityText = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        quantityText:SetPoint("BOTTOMLEFT", icon, "BOTTOMLEFT", 0, 0)
        ...
        -- Safely assign icon and tooltip
        local iconTexture = info.itemIcon or "Interface\\Icons\\INV_Misc_QuestionMark"
        ...
        iconBtn:SetScript("OnEnter", function()
            ...
            elseif info.itemName then
                GameTooltip:SetText(info.itemName, 1, 1, 1)
            else
    ...
    function Reserves:GetPlayersForItem(itemId)
        local players = {}
        for _, player in pairs(KRT_SavedReserves or {}) do
...
do
    ...
    function UpdateUIFrame(self, elapsed)
            LocalizeUIFrame()
            if Utils.periodic(self, frameName, updateInterval, elapsed) then
                _G[frameName.."sortAscending"]:SetChecked(addon.options.sortAscending)
                _G[frameName.."useRaidWarning"]:SetChecked(addon.options.useRaidWarning)
                _G[frameName.."announceOnWin"]:SetChecked(addon.options.announceOnWin)
                _G[frameName.."announceOnHold"]:SetChecked(addon.options.announceOnHold)
                _G[frameName.."announceOnBank"]:SetChecked(addon.options.announceOnBank)
                _G[frameName.."announceOnDisenchant"]:SetChecked(addon.options.announceOnDisenchant)
                _G[frameName.."lootWhispers"]:SetChecked(addon.options.lootWhispers)
                _G[frameName.."countdownRollsBlock"]:SetChecked(addon.options.countdownRollsBlock)
                _G[frameName.."screenReminder"]:SetChecked(addon.options.screenReminder)
                _G[frameName.."ignoreStacks"]:SetChecked(addon.options.ignoreStacks)
                _G[frameName.."showTooltips"]:SetChecked(addon.options.showTooltips)
                _G[frameName.."minimapButton"]:SetChecked(addon.options.minimapButton)
